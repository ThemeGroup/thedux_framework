 ...
 * The Shortcode
 */
function thedux_video_modal_shortcode( $atts ) {
	extract( 
		shortcode_atts( 
			array(
				'layout' => 'local-large',
...
add_shortcode( 'caviar_video_modal', 'thedux_video_modal_shortcode' );

/**
 * The VC Functions
 */
function thedux_video_modal_shortcode_vc() {
	vc_map( 
		array(
			"icon" => 'caviar-vc-block',
			"name" => esc_html__("Video Modal Embeds", 'caviar'),
			"base" => "caviar_video_modal",
			...
			"params" => array(
				...
				array(
					"type" => "dropdown",
					"heading" => esc_html__("Icon Color", 'caviar'),
					"param_name" => "colour",
					"value" => array(
...
add_action( 'vc_before_init', 'thedux_video_modal_shortcode_vc' );