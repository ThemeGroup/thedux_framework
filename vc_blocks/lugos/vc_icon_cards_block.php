	...
	if( 'standard' == $layout ){
		...
		$output = '
			<div class="feature feature-1 '. esc_attr($alignment) .'">
				<i class="icon--lugos icon '. esc_attr($icon) .'"></i>
		...
		$output = '
			<div class="feature feature-1 '. esc_attr($alignment) .'">
				<i class="icon--lugos color--primary icon--lg '. esc_attr($icon) .'"></i>
				<h4>'. htmlspecialchars_decode($title) .'</h4>
				'. wpautop(do_shortcode(htmlspecialchars_decode($content))) .'</div>
		';
		...
		$output = '
			<div class="stats-1">
				<div class="feature feature-1 text-center boxed boxed--lg">
					<i class="icon--lugos icon--lg '. esc_attr($icon) .' color--primary"></i>
		...
		$output = '
			<div class="feature feature-3 '. esc_attr($alignment) .'">
				<div class="feature__left">
					<i class="icon--lugos icon '. esc_attr($icon) .' color--primary"></i>
				</div>
				<div class="feature__right">
					<h5>'. htmlspecialchars_decode($title) .'</h5>
					'. wpautop(do_shortcode(htmlspecialchars_decode($content))) .'
...
add_shortcode( 'lugos_icon_box', 'thedux_icon_box_shortcode' );
...
function thedux_icon_box_shortcode_vc() {
	...
	vc_map( 
		array(
			"icon" => 'lugos-vc-block',
			"name" => esc_html__("Icon & Text", 'lugos'),
			"base" => "lugos_icon_box",
			...
			"params" => array(
				...
				array(
					"type" => "dropdown",
					"heading" => esc_html__("Icon Box Display Type", 'lugos'),
					"param_name" => "layout",
					"value" => array(
						'Standard' => 'standard',
						'Small' => 'small',
				...
				array(
					"type" => "dropdown",
					"heading" => esc_html__("Alignment", 'lugos'),
