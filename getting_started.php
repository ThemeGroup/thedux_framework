...
if(!( function_exists('thedux_getting_started_add_page') )){
	function thedux_getting_started_add_page(){
		add_theme_page(
			esc_html__('Getting Started', 'thedux_framework'),
			esc_html__('Getting Started', 'thedux_framework'),
			'manage_options',
			'thedux_framework-getting-started',
			'thedux_getting_started_page'
		);
	}
	add_action('admin_menu', 'thedux_getting_started_add_page');
}

if(!( function_exists('thedux_start_load_admin_scripts') )){
	function thedux_start_load_admin_scripts() {
		 ...
		 * @since 1.0
		 */
		wp_enqueue_style('thedux_framework-getting-started', plugins_url( '/css/getting-started-page.css' , __FILE__ ) );
		wp_enqueue_script('thedux_framework-getting-started', plugins_url( '/js/getting-started-page.js' , __FILE__ ) );
	}
	add_action( 'admin_enqueue_scripts', 'thedux_start_load_admin_scripts' );
}
 ...
 * since 1.0.0
 */
if(!( function_exists('thedux_getting_started_page') )){
	function thedux_getting_started_page(){
	...
	<div class="wrap getting-started">
		...
		<div class="intro-wrap">
			...
			<div class="intro">
				<h3><?php printf( esc_html__( 'Getting started with %1$s v%2$s', 'thedux_framework' ), $theme['Name'], $theme['Version'] ); ?></h3>
	
				<h4><?php printf( esc_html__( 'Thanks for purchasing %1$s. You will find everything you need to get started setting up your site below.', 'thedux_framework' ), $theme['Name'] ); ?></h4>
			</div>
		</div>
	
		<div class="panels">
			<ul class="inline-list">
				<li class="current"><a id="help" href="#"><i class="fa fa-check"></i> <?php esc_html_e( 'Help File', 'thedux_framework' ); ?></a></li>
				<li><a id="plugins" href="#"><i class="fa fa-plug"></i> <?php esc_html_e( 'Plugins', 'thedux_framework' ); ?></a></li>
				<li><a id="support" href="#"><i class="fa fa-life-ring"></i> <?php esc_html_e( 'Troubleshooting', 'thedux_framework' ); ?></a></li>
				<li><a id="updates" href="#"><i class="fa fa-question-circle"></i> <?php esc_html_e( 'FAQ &amp; Support', 'thedux_framework' ); ?></a></li>
			</ul>
			...
			<div id="panel" class="panel">
				...
				<div id="help-panel" class="panel-left visible">
					<?php
						$rss = wp_remote_get('https://dzungnova.ticksy.com/articles/'. $this_id .'/?print');
						if(!( is_wp_error($rss) )){
							echo wp_kses($rss['body'], $my_allowed);
						} else {
							echo 'Help file currently unavailable, <a href="https://dzungnova.ticksy.com/articles/">view directly here</a>.';
						}
					?>
				</div>
				...
				<div id="plugins-panel" class="panel-left">
					<h3><?php esc_html_e( 'Required Plugins', 'thedux_framework' ); ?></h3>
	
					<p><?php esc_html_e( 'Your new theme has a list of required and recommended plugins that it may need to run. To install these visit "appearance => install plugins", if this menu option is unavailable then you have already installed the plugins, well done!', 'thedux_framework' ); ?></p>
					...
					<p>For more tips on how to speed up for you WordPress site, please see <a href="http://www.dzungnova.com/speeding-up-wordpress/">Speeding Up WordPress</a>.</p>
					...
					<p>We have a more in depth view of WordPress SEO over in our article, <a href="http://www.dzungnova.com/boost-your-sites-seo/">Boost Your Sites SEO</a>.</p>
					...
					<p><a href="https://en-gb.wordpress.org/plugins/contact-form-7/">Contact Form 7</a>&nbsp;has been our go-to plugin in our <a href="http://themeforest.net/user/dzungnova?ref=dzungnova">Premium WordPress Themes</a>&nbsp;for a long time, and the reason is it simply works, and works well. Forms are created with a simple to follow interface, and offer full logic control and many input styles so almost every type of form can be handled with ease. Once you have created your ideal form, your provided a shortcode which can be placed almost anywhere on your site so its the most flexible solution with zero cost to yourself, ideal!</p>
					...
					<p>Once you have the foundations in place provided by the plugins above, you can continue customization your by viewing some of our other blog posts. Perhaps you want to give your Dashboard a personal touch? Then check out <a href="http://www.dzungnova.com/spicing-up-your-wordpress-dashboard/">Spicing Up Your WordPress Dashboard</a>.</p>
					<p>And of course all sites should have a backup solution in place, so if you havn’t done so already, you check check out <a href="http://www.dzungnova.com/wp-basics-setting-up-a-backuprestore-system/">our post on that too</a>.</p>
				...
				<div id="support-panel" class="panel-left">
					<?php
						$rss = wp_remote_get('https://dzungnova.ticksy.com/article/7588/?print');
						if(!( is_wp_error($rss) )){
							echo wp_kses($rss['body'], $my_allowed);
						} else {
							esc_html_e( 'Help file currently unavailable.', 'thedux_framework' );
						}
					?>
				...
				<div id="updates-panel" class="panel-left">
					...
					<h3 id="faq-support"><?php esc_html_e( 'Where do I get support for ', 'thedux_framework' ); echo $theme['Name']; ?>?</h3>
				...
				<div class="panel-right">
					...
					<div class="panel-aside">
						<h4><?php esc_html_e( 'Visit the Knowledge Base', 'thedux_framework' ); ?></h4>
						<p><?php esc_html_e( 'New to the WordPress world? Our Knowledge Base has written & video tutorials, from installing WordPress to working with themes and more.', 'thedux_framework' ); ?></p>
	
						<a class="button button-primary" href="https://dzungnova.ticksy.com/articles/"><?php esc_html_e( 'Visit the Knowledge Base', 'thedux_framework' ); ?></a>
