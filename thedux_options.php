...
class Themedeux_Options {
	...
	public function __construct(){
		add_action('customize_register', 'thedux_additional_classes', 10 );
		...
		if( is_array($this->settings) ){
			foreach( $this->settings as $setting ){
				...
				if( 'image' == $type ){
					...
					$wp_customize->add_control( 
						new Themedeux_Customizer_Number_Control(
							$wp_customize, $name, array(
					    		'label'    => $title,
					    		'section'  => $section,
					    		'priority' => $priority
					...
					$wp_customize->add_control( 
						new Themedeux_Customizer_Textarea_Control(
							$wp_customize, $name, array(
					    		'label'    => $title,
					    		'section'  => $section,
					    		'priority' => $priority
					...
					$wp_customize->add_control( 
						new Themedeux_Customizer_Range_Control(
							$wp_customize, $name, array(
					    		'label'    => $title,
					    		'section'  => $section,
					    		'priority' => $priority,
					...
					$wp_customize->add_control( 
						new Themedeux_Customizer_Demo_Import_control( 
							$wp_customize, $name, array(
							    'label'    => $title,
							    'section'  => $section,
							    'priority' => $priority
...
}//end Themedeux_Options

if(!( function_exists('thedux_additional_classes') )){
	function thedux_additional_classes(){
		
		class Themedeux_Customizer_Range_Control extends WP_Customize_Control {
		    public $type = 'range';
		    public function render_content() {
		    ?>
		        <label>
		        	<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
		        	<input <?php $this->link(); ?> name="<?php echo esc_html( thedux_sanitize_title($this->label) ); ?>" type="range" min="<?php echo $this->choices['min']; ?>" max="<?php echo $this->choices['max']; ?>" step="<?php echo $this->choices['step']; ?>" value="<?php echo intval( $this->value() ); ?>" class="thedux-range" onchange="printValue('<?php echo esc_html( thedux_sanitize_title($this->label) ); ?>')" />
		        	<input type="text" name="<?php echo esc_html( thedux_sanitize_title($this->label) ); ?>" class="thedux-range-output" value="<?php echo intval( $this->value() ); ?>" disabled/>
		        </label>
		    <?php
		    }
		}
		
		class Themedeux_Customizer_Textarea_Control extends WP_Customize_Control {
		...
		class Themedeux_Customizer_Number_Control extends WP_Customize_Control {
			public $type = 'number';
			public function render_content() {
			?>
				<label>
		...
		class Themedeux_Customizer_Demo_Import_control extends WP_Customize_Control {
